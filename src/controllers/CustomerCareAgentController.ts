...
export const LoginCCA = async (req: Request, res: Response) => {
	try {
		const validationResponse = CustomerCareAgentLoginSchema.safeParse(
			req.body
		);
		if (!validationResponse.success) {
			res.status(400).send(validationResponse.error.format());
...
const getOffsetPagination = (query: any) => {
	const offset = isNaN(Number(query.offset))
		? OFFSET
...
export const requestPasswordResetResponse = async (
	...
	try {
		const { id, token } = req.params;
		const validationResponse = PasswordResetResponseSchema.safeParse(
			req.body
		);
		if (!validationResponse.success) {
			return res.status(400).send(validationResponse.error.format());
		...
		if (userExists && userExists.user) {
			const tokenObject =
				await prisma.customerCareAgentResetToken.findFirst({
					where: {
						userId: id,
						token: token,
					},
			...
			return res
				.status(200)
				.json({ message: 'Reset successful', user: agent });
		}
		return res.status(400).send({ message: 'Invalid link' });
	} catch (error) {
		logger.error(
			'Error in email reset response verifying new CCA: ',
			error
		);
		return res.status(500).send({ error: 'Internal Server Error' });
	}
